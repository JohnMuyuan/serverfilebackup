...
6. 管理后台备份（进度/日志/取消/定时器）
7. 删除任务（保留归档）
0. 退出
```

菜单 `3` 和 `4` 会把任务交给 systemd 后台执行，启动后可以安全关闭 SSH。菜单 `6` 会列出正在运行或失败的任务，可实时跟踪日志、查看最近日志、一键停止任务或查看下次定时运行时间。停止任务会保留 rsync 的断点续传数据，并删除未完成的 `.tar.gz.part` 临时归档。
